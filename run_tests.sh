...
if echo "SELECT 1 FROM duckdb_functions() WHERE function_name = 'align_minibwa';" | ./build/release/duckdb -csv -noheader 2>/dev/null | grep -q 1; then
    export MINIBWA_AVAILABLE=1
fi
