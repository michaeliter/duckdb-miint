...
- **`src/krepp.cpp` is excluded** — it holds `main()` and the CLI11 argument layer. Two consequences worth knowing:
  ...
  It also converts one case krepp cannot: two indexes in one directory. `krepp index` never clears what is already there, so re-indexing with different `-h`/`-w` leaves both file sets behind, and krepp notices only inside `Index::check_compatible` (`index.cpp:26`, `:47`, `:86`) — `error_exit`. The filename suffix decides it first, through `krepp_detail::PartialHashConfig`: `m` and `frac` must agree across partials, and so must `r` when `frac` is true, mirroring `LSHF::check_compatible` (`lshf.cpp:163-170`); under `frac := false` the residue is the partial id and is expected to differ.
