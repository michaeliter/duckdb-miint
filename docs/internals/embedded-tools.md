...
- **Location:** `ext/rammap/` — **currently a local source vendor, not a submodule** (no `.git` of its own; copied from `jwanglab/rammap` minus the `rammap` CLI crate and `web/` demo). `ext/rammap/Cargo.toml` documents this and the trimmed workspace `members` list. `RAMMAP_GIT_VERSION` is hardcoded (`"vendored-1.1.2-local"`) rather than `git describe`d — replace both once this becomes a real submodule (a `the-miint/rammap` fork or a pinned upstream tag).
- **Purpose:** pure-Rust minimap2-compatible aligner, exposed as `align_rammap`. Unlike minimap2 (embedded via its own C library + hand-reconstructed SAM fields in `Minimap2Aligner.cpp`), rammap is held to a stricter bar: byte-identical output to the standalone `rammap` CLI. See `test/sql/align_rammap_parity.test`.
- **Gated by:** `MIINT_ENABLE_RAMMAP` (default on native Linux/macOS; auto-off on Emscripten and Windows — unverified against the umbrella glue crate's WASM/MinGW cargo target flags). `MIINT_HAS_RAMMAP` compile define when on.
- **No FFI of its own:** unlike rype/sylph (which each ship an `arrow-ffi` feature upstream), `rammap-core` exposes no C ABI at all — only a pure Rust `api::Aligner` and a WASM shim. The extern "C" surface lives in **this repo's own** `ext/miint-rust-glue/src/rammap_ffi.rs`, gated by the glue crate's `with-rammap` feature.
- **Parity strategy (why output matches byte-for-byte):** the standalone `rammap` CLI's SAM formatting is not CLI-only code — `rammap/src/main.rs` calls the public `rammap::align::pipeline::align_and_format_query`, which returns a `String` of SAM text. `rammap_ffi.rs`'s `rammap_align_query` calls that exact same function and returns the SAM text verbatim; `RammapAligner.cpp` parses it with htslib's `sam_parse1` — the same text-SAM parser MIINT uses for on-disk SAM files (`SAMReader.cpp`) — into a `SAMRecordBatch` via the existing `sam_utils::parse_record_to_batch` helper (`SAMRecord.hpp`). No alignment field is reconstructed in C++; this is the entire reason parity holds by construction rather than by coincidence.
- **Option-derivation parity:** `rammap_index_build` (in `rammap_ffi.rs`) mirrors the CLI's `run()` option construction line-for-line (preset → k/w override → the CLI's *unconditional* `chn_pen_gap`/`chn_pen_skip` recompute → `Index::build` → the CLI's *conditional* `cal_mid_occ` — some presets like `"sr"` set a fixed `mid_occ` and skip recalculation entirely, which the FFI replicates via the same `if mid_occ == 0` guard). Any change to that derivation is the first thing to suspect if the parity test ever regresses.
- **Non-native SAM tags:** rammap's own SAM output doesn't emit bowtie2-style `XM`/`XO`/`XG`/`YT` tags. Rather than reconstruct them in C++ (the class of divergence bug `align_minimap2` has), `align_rammap` leaves those columns `NULL` — see `test/sql/align_rammap.test`. `AS`/`NM`/`MD`/`SA` are populated directly from rammap's own tags.
- **v1 scope:** `align_rammap` supports `subject_table` (shared-index, multi-threaded) only — no `index_path` (rammap-core has no `.mmi`/`.rmmi`-file-loading FFI entry point yet) and no `per_subject_database` mode. Single-end queries only; a paired-end query throws (rammap's `align_and_format_pair` isn't wired up yet).
- **C++ wrappers:** `src/include/RammapAligner.hpp` / `src/RammapAligner.cpp` — `SharedRammapIndex` (RAII over the FFI's opaque index handle, built once from subjects, shared read-only across worker threads) + `RammapAligner` (RAII over a per-thread FFI context handle; NOT thread-safe, one instance per worker thread — mirrors `Minimap2Aligner`/`SharedMinimap2Index`). `src/include/rammap_ffi.h` is the hand-written C declaration header for the Rust extern "C" surface (mirrors `ext/sylph/sylph.h`'s role).
