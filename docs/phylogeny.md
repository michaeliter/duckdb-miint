...
- `m` (INTEGER, default 4), `r` (INTEGER, default 1), `frac` (BOOLEAN, default true): Partitioning of the *k*-mer space, exactly as krepp's `--m`, `--r` and `--frac`. These decide the index's filename suffix (`-m4r1-frac`).
  ...
  **`r` means different things depending on `frac`, and only one of them partitions.** With `frac := false`, a *k*-mer is indexed when `LSH(x) mod m == r`, so each `r` is a disjoint slice. With `frac := true` — the default, matching krepp's own — the test is `LSH(x) mod m <= r` (`ext/krepp/src/rqseq.cpp:133`), making `r` a cumulative **threshold**: `r := 2` contains everything `r := 1` contains and more, and the partial is registered under every residue from 0 to `r` (`ext/krepp/src/index.cpp:146-148`), overwriting smaller ones as they load. Measured on the 25-genome corpus in `ext/krepp/test` at `m := 4`, `total_num_kmers` runs 8,675,085 / 13,491,823 / 17,366,632 for `r` = 1 / 2 / 3 — `r := 3` already spans the whole space. Placing 40 reads against a directory holding `r` = 1–4 gave output identical to `r = 4` alone; the other three partials contributed nothing.
